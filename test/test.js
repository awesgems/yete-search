...
test( 'playlist metadata by id', function ( t ) {
  ...
  yts( { listId: 'PL7k0JFoxwvTbKL8kjGI_CaV31QxCGf1vJ' }, function ( err, playlist ) {
    ...
    if ( playlist.videos[ 0 ].duration.seconds === ( 60 * 1 + 37 ) ) {
      ...
      t.equal( playlist.videos[ 5 ].duration.seconds, 60 * 6 + 45, 'play list video 2 duration.seconds ok' )
      t.equal( playlist.videos[ 5 ].duration.timestamp, '6:45', 'play list video 2 duration.timestamp ok' )
